...
import { escortNameFor } from "./geometry";
...
/**
 * 全ゲストぶんのエスコートカード(チケット風)を実寸 182×65mm のまま、
 * A4 1 枚に 4 枚ずつ並べてまとめて印刷する。1枚の画像化に失敗しても
...
export async function printAllEscortCards({
  ...
  if (!w) return { opened: false, printed: 0, failed: 0 };
  w.document.write(
    printLoadingHtml({
  ...
  for (let i = 0; i < letters.length; i++) {
    if (w.closed) return { opened: false, printed: images.length, failed };
    try {
      images.push(await captureOne(letters[i], escortConf, fontFamily, fallbackFootText));
    } catch (err) {
      failed++;
      console.error("escort card capture failed", letters[i].id, err);
    }
    onProgress?.(i + 1, letters.length);
    if (!w.closed) {
      setProgress(w, i + 1, letters.length, failed);
    }
  }
  if (w.closed) return { opened: false, printed: images.length, failed };
  ...
  if (images.length === 0) {
    ...
    w.document.close();
    return { opened: true, printed: 0, failed };
  }
  ...
  for (let i = 0; i < images.length; i += CARDS_PER_PAGE) {
    const cardsHtml = images
      ...
      .map((src) => `<img class="card" src="${src}" />`)
      .join("");
    pages.push(`<div class="page">${cardsHtml}</div>`);
  }

  w.document.open();
  w.document.write(`<!doctype html><html><head><title>エスコートカード一括印刷</title><style>
